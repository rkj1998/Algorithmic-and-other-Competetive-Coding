...
class ChefAndMeetings {
  ...
  static int stringTimetoIntTime(String hrtime){
        int time =0;
        if(hrtime.substring(6,8).equals("AM")){
            time = Integer.parseInt((hrtime.substring(0, 2)));
            if(time==12){
                time=0;
            }
            time*=10;
            time+=Integer.parseInt((hrtime.substring(3, 4)));
            time*=10;
            time+=Integer.parseInt((hrtime.substring(4, 5)));

        }
        else{
            time = Integer.parseInt((hrtime.substring(0, 2)))+12;
            if(time==24){
                time=12;
            }
            time*=10;
    ...
    public static void main(String[] args) {
        try {
            ...
            for(int i=0;i<t;i++){
                ...
                int meetTIme = stringTimetoIntTime(time);
                int n = Integer.parseInt(read.nextLine());
                int result[] = new int[n];
                for(int j=0;j<n;j++){
                    String totalTime = read.nextLine();
                    int startTime= stringTimetoIntTime(totalTime);
                    int endTime = stringTimetoIntTime(totalTime.substring(9));
                    if(startTime<=meetTime<=endTime){
                        result[j]=1;
                    }
